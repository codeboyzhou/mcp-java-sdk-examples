...
public final class McpPrompts {
	 ...
	 * @return The prompt message, wrapped as a
	 * {@link McpServerFeatures.SyncPromptSpecification} object.
	 * @see McpTools#find()
	 */
	public static McpServerFeatures.SyncPromptSpecification find() {
		...
		McpSchema.PromptArgument start = new McpSchema.PromptArgument("start", "The starting path to search, required.",
				true);
		McpSchema.PromptArgument name = new McpSchema.PromptArgument("name",
				"The name of the target file or directory to search, supports fuzzy matching, required.", true);

		// Step 2: Create a prompt with name, description, and arguments.
		List<McpSchema.PromptArgument> args = List.of(start, name);
		McpSchema.Prompt prompt = new McpSchema.Prompt("find",
	...
	public static McpServerFeatures.SyncPromptSpecification read() {
		// Step 1: Create a prompt argument with name, description, and required flag.
		McpSchema.PromptArgument path = new McpSchema.PromptArgument("path",
				"The path to read, can be a file or directory, required.", true);

		// Step 2: Create a prompt with name, description, and arguments.
		List<McpSchema.PromptArgument> args = List.of(path);
		McpSchema.Prompt prompt = new McpSchema.Prompt("read",
	...
	public static McpServerFeatures.SyncPromptSpecification delete() {
		// Step 1: Create a prompt argument with name, description, and required flag.
		McpSchema.PromptArgument path = new McpSchema.PromptArgument("path",
				"The path to delete, can be a file or directory, required.", true);

		// Step 2: Create a prompt with name, description, and arguments.
		List<McpSchema.PromptArgument> args = List.of(path);
		McpSchema.Prompt prompt = new McpSchema.Prompt("delete", "Delete a file or directory from the filesystem.",
				args);
